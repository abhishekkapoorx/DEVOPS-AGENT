...
class CodebaseAnalyzer(BaseTool):
    """Tool for analyzing codebase structure and dependencies."""
    
    name: str = "analyze_codebase"
    description: str = "Analyze a codebase to detect programming languages, frameworks, dependencies, and project structure for Docker optimization"
    args_schema: Type[BaseModel] = CodebaseAnalysisInput
        ...
        for root, dirs, files in os.walk(directory_path):
            ...
            for file in files:
                ...
                if file == 'requirements.txt':
                    dependencies['python'] = self._parse_requirements(file_path)
                elif file == 'package.json':
                    dependencies['node'] = self._parse_package_json(file_path)
                elif file == 'pom.xml':
                    dependencies['java'] = self._parse_pom_xml(file_path)
                elif file == 'go.mod':
                    dependencies['go'] = self._parse_go_mod(file_path)
                elif file == 'Cargo.toml':
                    dependencies['rust'] = self._parse_cargo_toml(file_path)

                # Detect frameworks
                if file == 'package.json':
                    frameworks.extend(self._detect_js_frameworks(file_path))
                elif file == 'requirements.txt':
                    frameworks.extend(self._detect_python_frameworks(file_path))
    ...
    def _detect_js_frameworks(self, package_json_path: str) -> List[str]:
    ...
    def _detect_python_frameworks(self, requirements_path: str) -> List[str]:
...
class DockerfileGenerator(BaseTool):
    """Tool for generating optimized Dockerfiles based on codebase analysis."""
    
    name: str = "generate_dockerfile"
    description: str = "Generate a production-ready Dockerfile based on codebase analysis results"
    args_schema: Type[BaseModel] = DockerfileInput
    _model: Any = PrivateAttr(default=None)
    
    def __init__(self):
        super().__init__()
        # Import LLM models here to avoid circular imports
        from llms import openai_models
        self._model = openai_models["gpt-4o-mini"]
    
    def _run(self, analysis_result: str, output_path: str, optimization_level: str = "production") -> str:
        """Generate Dockerfile based on analysis using LLM."""
        try:
            analysis = json.loads(analysis_result)
            dockerfile_content = self._generate_dockerfile_with_llm(analysis, optimization_level)
    ...
    def _generate_dockerfile_with_llm(self, analysis: Dict[str, Any], optimization_level: str) -> str:
...
"""
        ...
        try:
            ...
            return self._strip_dockerignore_from_dockerfile(content)
        except Exception as e:
            # No fallback: propagate error to caller
            raise e

    def _strip_dockerignore_from_dockerfile(self, content: str) -> str:
        ...
        """Simplest generic placeholder; prefer LLM path for real stacks."""
        return """
FROM scratch
# Unable to infer stack; please provide app runtime details.
"""
...
class DockerComposeGenerator(BaseTool):
    """Tool for generating docker-compose.yml files based on codebase analysis."""
    
    name: str = "generate_docker_compose"
    description: str = "Generate a docker-compose.yml file based on codebase analysis results"
    args_schema: Type[BaseModel] = DockerComposeInput
    
    def _run(self, analysis_result: str, output_path: str, environment: str = "development", services: List[str] = None) -> str:
        ...
        try:
            ...
            analysis = json.loads(analysis_result)
            compose_content = self._generate_compose_content(analysis, environment, services)
    ...
    def _generate_compose_content(self, analysis: Dict[str, Any], environment: str, additional_services: List[str]) -> str:
        ...
        if 'Python' in languages:
            services['app'] = self._get_python_service(environment)
        elif 'JavaScript/TypeScript' in languages:
            services['app'] = self._get_node_service(environment)
        elif 'Java' in languages:
            services['app'] = self._get_java_service(environment)
        elif 'Go' in languages:
            services['app'] = self._get_go_service(environment)
        else:
            services['app'] = self._get_generic_service(environment)
        
        # Add database services based on frameworks
        if 'Django' in frameworks or 'Flask' in frameworks:
            services['db'] = self._get_postgres_service(environment)
        elif 'React' in frameworks or 'Vue.js' in frameworks:
            services['frontend'] = self._get_frontend_service(environment)
        
        # Add additional services
        for service in additional_services:
            if service.lower() == 'postgres':
                services['postgres'] = self._get_postgres_service(environment)
            elif service.lower() == 'redis':
                services['redis'] = self._get_redis_service(environment)
            elif service.lower() == 'nginx':
                services['nginx'] = self._get_nginx_service(environment)
            elif service.lower() == 'mongodb':
                services['mongodb'] = self._get_mongodb_service(environment)
...
"""
    ...
    def _get_python_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_node_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_java_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_go_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_generic_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_postgres_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_redis_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_nginx_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_mongodb_service(self, environment: str) -> Dict[str, Any]:
    ...
    def _get_frontend_service(self, environment: str) -> Dict[str, Any]:
